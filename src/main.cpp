...
void write_atoms_to_xtc( gsl_matrix *cell , CMMDBManager *mmdb , std::string fname, std::vector<int> vimnr) 
{
...
// BELOW IS FOR TESTING 
	...
	do {
		...
		if(k==0 && verbose >= 1 ) { // check coordinates in first frame
			mmdb->Select (	selHnd  , STYPE_ATOM , vimnr[k] , "*",
					ANY_RES , "*"	, ANY_RES,"*" ,
					"*", "*", "*" 	, "*", SKEY_NEW );
			mmdb->Select (	selHnd  , STYPE_ATOM , vimnr[k] , "*",
...
//	START TO WORK WITH THE DATA (TABLES)
	...
	std::string fstr("traj.xtc");
	write_atoms_to_xtc( cell , &mmdb , fstr , vimnr);
